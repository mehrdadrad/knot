        ...
        for (int i = KNOT_ANSWER; i <= KNOT_AUTHORITY; i++) {
                ...
                for (int j = 0; j < section->count; j++) {
                        ...
                        for (int k = 0; a != NULL && k < a->count; k++) {
                                if (a->glues[k].node->owner == rr->owner) { // no need for knot_dname_cmp because the pointers are assigned
                                        return &a->glues[k];
                                }
                        }
                }
        }
        assert("find_glue_for" && 0);
        return NULL;
}
...
static bool shall_sign_rr(const knot_rrset_t *rr, const knot_pkt_t *pkt)
{
        if (pkt->current == KNOT_ADDITIONAL) {
                glue_t *g = find_glue_for(rr, pkt); // finds actually the node which is rr in
			       ...
			       knot_mm_t *mm)
{
	...
	int r = knot_rrset_add_rdata(ds, rdata.data, rdata.size, mm);
	if (r != KNOT_EOK) {
		knot_rrset_free(&ds, mm);
		return NULL;
	}
